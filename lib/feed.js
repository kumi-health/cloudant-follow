...
Feed.prototype.follow = function follow_feed() {
  ...
  if (self.feed !== 'continuous' && self.feed !== 'longpoll') { throw new Error('The only valid feed options are "continuous" and "longpoll"'); }
...
Feed.prototype.on_couch_data = function on_couch_data(change) {
  ...
  if (!self.is_db_updates && !change.seq) { return self.die(new Error('Change has no .seq field: ' + JSON.stringify(change))); }
...
Feed.prototype.on_change = function on_change(change) {
  ...
  if (!self.is_db_updates && !change.seq) { return self.die(new Error('No seq value in change: ' + lib.JS(change))); }
