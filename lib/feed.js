...
Feed.prototype.query = function query_feed() {
  ...
  var feed_request = self.httpAgent(req, (error, response) => {
    console.log("CLOUDANT-FOLLOW")
    if (error) {
      console.error("CLOUDANT-FOLLOW ERROR: ", error)
    } else {
      console.error("CLOUDANT-FOLLOW RESPONSE: ", response)
    }
  });
